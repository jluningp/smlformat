structure AddComments : ADD_COMMENTS = struct
  ...
  fun addComment (conversionInfo : conversionInfo) value convert comment mark region
    ...
    let
        ...
        val attachedComments =
          List.filter (fn (commentLine, comments) => line >= commentLine)
            (! comments)
  ...
  and convertStrexp conversionInfo strexp =
      case strexp of
          Ast.AppStr (path, strexps) =>
            AppStr
              (path
              , List.map (fn (strexp, bool) => (convertStrexp conversionInfo strexp, bool))
                strexps)
        ...
        | Ast.BaseFct { body : Ast.strexp
          ...
          , params : (symbol option * Ast.sigexp) list } =>
            BaseFct
              ...
              , params = List.map (fn (sym, sigexp) => (sym, convertSigexp conversionInfo sigexp))
                params }
  ...
  and convertFsigexp conversionInfo fsigexp =
      case fsigexp of
          Ast.BaseFsig { param : (symbol option * Ast.sigexp) list, result : Ast.sigexp } =>
            BaseFsig
              { param = List.map (fn (sym, sigexp) => (sym, convertSigexp conversionInfo sigexp))
                param
              , result = convertSigexp conversionInfo result }
        | Ast.MarkFsig (fsigexp, region) =>
            addComment conversionInfo fsigexp convertFsigexp CommentFsig MarkFsig
              region
  ...
  and convertSpec conversionInfo spec =
      case spec of
        ...
        | Ast.ExceSpec tys =>
            ExceSpec
              (List.map (fn (sym, ty) => (sym, Option.map (convertTy conversionInfo) ty))
                tys)
  ...
  and convertDb
    ...
    Db
        { lazyp = lazyp
        , rhs = List.map (fn (sym, ty) => (sym, Option.map (convertTy conversionInfo) ty))
          rhs
        , tyc = tyc
        , tyvars = List.map (convertTyvar conversionInfo) tyvars }
