structure AddComments : ADD_COMMENTS = struct
  ...
  fun addComment (conversionInfo : conversionInfo) value convert comment mark region =
      ...
      in
        case attachedComments of
          ...
          | comments =>
              comment
              (String.concat (List.map (fn (_, x) => String.concat x) comments), mark (converted, region))
      end
  ...
  and convertFctexp conversionInfo fctexp =
      case fctexp of
        ...
        | Ast.BaseFct {body : Ast.strexp, constraint : Ast.sigexp Ast.sigConst, params : (symbol option * Ast.sigexp) list} =>
            BaseFct
  ...
  and convertFsigexp conversionInfo fsigexp =
      case fsigexp of
        ...
        | Ast.MarkFsig (fsigexp, region) =>
            addComment conversionInfo fsigexp convertFsigexp CommentFsig MarkFsig region
  ...
  and convertRvb conversionInfo rvb =
      case rvb of
        ...
        | Ast.Rvb {exp : Ast.exp, fixity : (symbol * region) option, lazyp : bool, resultty : Ast.ty option, var : symbol} =>
            Rvb
  ...
  and convertDb conversionInfo (Ast.Db {lazyp : bool, rhs : (symbol * Ast.ty option) list, tyc : symbol, tyvars : Ast.tyvar list}) =
      Db
