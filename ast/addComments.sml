structure AddComments = struct
    ...
    fun convertExp (conversionInfo : conversionInfo) exp =
        case exp of
          ...
          | Ast.AppExp {argument = argument : Ast.exp, function = function : Ast.exp} =>
              AppExp
              {argument = convertExp conversionInfo argument, function = convertExp conversionInfo function}
          | Ast.CaseExp {expr = expr : Ast.exp, rules = rules : Ast.rule list} =>
              CaseExp
              {expr = convertExp conversionInfo expr, rules = List.map (convertRule conversionInfo) rules}
          | Ast.CharExp str => CharExp str
          | Ast.ConstraintExp {constraint = constraint : Ast.ty, expr = expr : Ast.exp} =>
              ConstraintExp
              {constraint = convertTy conversionInfo constraint, expr = convertExp conversionInfo expr}
          | Ast.FlatAppExp exps =>
              FlatAppExp (List.map (mapFixitem (convertExp conversionInfo)) exps)
          | Ast.FnExp rules => FnExp (List.map (convertRule conversionInfo) rules)
          | Ast.HandleExp {expr = expr : Ast.exp, rules = rules : Ast.rule list} =>
              HandleExp
              {expr = convertExp conversionInfo expr, rules = List.map (convertRule conversionInfo) rules}
          | Ast.IfExp {elseCase = elseCase : Ast.exp, test = test : Ast.exp, thenCase = thenCase : Ast.exp} =>
              IfExp
              {elseCase = convertExp conversionInfo elseCase, test = convertExp conversionInfo test, thenCase = convertExp conversionInfo thenCase}
          | Ast.IntExp literal => IntExp literal
          | Ast.LetExp {dec = dec : Ast.dec, expr = expr : Ast.exp} =>
              LetExp {dec = convertDec conversionInfo dec, expr = convertExp conversionInfo expr}
          ...
          | Ast.WhileExp {expr = expr : Ast.exp, test = test : Ast.exp} =>
              WhileExp
    ...
    and convertPat conversionInfo pat =
        case pat of
            Ast.AppPat {argument = argument : Ast.pat, constr = constr : Ast.pat} =>
              AppPat
              {argument = convertPat conversionInfo argument, constr = convertPat conversionInfo constr}
          | Ast.CharPat str => CharPat str
          | Ast.ConstraintPat {constraint = constraint : Ast.ty, pattern = pattern : Ast.pat} =>
              ConstraintPat
              {constraint = convertTy conversionInfo constraint, pattern = convertPat conversionInfo pattern}
          | Ast.FlatAppPat pats =>
              FlatAppPat (List.map (mapFixitem (convertPat conversionInfo)) pats)
          | Ast.IntPat l => IntPat l
          | Ast.LayeredPat {expPat = expPat : Ast.pat, varPat = varPat : Ast.pat} =>
              LayeredPat
          ...
          | Ast.RecordPat {def = def : (symbol * Ast.pat) list, flexibility = flexibility : bool} =>
              RecordPat
              {def = List.map (fn (sym, pat) => (sym, convertPat conversionInfo pat)) def, flexibility = flexibility}
    ...
    and convertStrexp conversionInfo strexp =
        case strexp of
            Ast.AppStr (path, strexps) =>
              AppStr
              (path, List.map (fn (strexp, bool) => (convertStrexp conversionInfo strexp, bool)) strexps)
          ...
          | Ast.ConstrainedStr (strexp, sigConst) =>
              ConstrainedStr
              (convertStrexp conversionInfo strexp, mapSigconst (convertSigexp conversionInfo) sigConst)
    ...
    and convertFctexp conversionInfo fctexp =
        case fctexp of
            Ast.AppFct (path, strexps, sigConst) =>
              AppFct
              (path, List.map (fn (strexp, bool) => (convertStrexp conversionInfo strexp, bool)) strexps, mapSigconst (convertFsigexp conversionInfo) sigConst)
          | Ast.BaseFct {body = body : Ast.strexp, constraint = constraint : Ast.sigexp Ast.sigConst, params = params : (symbol option * Ast.sigexp) list} =>
              BaseFct
              {body = convertStrexp conversionInfo body, constraint = mapSigconst (convertSigexp conversionInfo) constraint, params = List.map (fn (sym, sigexp) => (sym, convertSigexp conversionInfo sigexp)) params}
    ...
    and convertSigexp conversionInfo sigexp =
        case sigexp of
            Ast.AugSig (sigexp, wherespecs) =>
              AugSig
              (convertSigexp conversionInfo sigexp, List.map (convertWherespec conversionInfo) wherespecs)
    ...
    and convertFsigexp conversionInfo fsigexp =
        case fsigexp of
            Ast.BaseFsig {param = param : (symbol option * Ast.sigexp) list, result = result : Ast.sigexp} =>
              BaseFsig
              {param = List.map (fn (sym, sigexp) => (sym, convertSigexp conversionInfo sigexp)) param, result = convertSigexp conversionInfo result}
          | Ast.MarkFsig (fsigexp, region) =>
              MarkFsig (convertFsigexp conversionInfo fsigexp, region)
          | Ast.VarFsig sym => VarFsig sym

    and convertSpec conversionInfo spec =
        case spec of
            Ast.DataReplSpec (symbol, path) => DataReplSpec (symbol, path)
          | Ast.DataSpec {datatycs = datatycs : Ast.db list, withtycs = withtycs : Ast.tb list} =>
              DataSpec
              {datatycs = List.map (convertDb conversionInfo) datatycs, withtycs = List.map (convertTb conversionInfo) withtycs}
          | Ast.ExceSpec tys =>
              ExceSpec
              (List.map (fn (sym, ty) => (sym, Option.map (convertTy conversionInfo) ty)) tys)
          | Ast.FctSpec fsigexps =>
              FctSpec
              (List.map (fn (sym, fsigexp) => (sym, convertFsigexp conversionInfo fsigexp)) fsigexps)
          ...
          | Ast.StrSpec strs =>
              StrSpec
              (List.map (fn (sym, sigexp, path) => (sym, convertSigexp conversionInfo sigexp, path))
                strs)
          | Ast.TycSpec (tys, b) =>
              TycSpec
              (List.map
                (fn (sym, tyvars, ty) =>
                    (sym, List.map (convertTyvar conversionInfo) tyvars, Option.map (convertTy conversionInfo) ty))
                tys, b)
          | Ast.ValSpec tys =>
    ...
    and convertDec conversionInfo dec =
        case dec of
          ...
          | Ast.AbstypeDec {abstycs = abstycs : Ast.db list, body = body : Ast.dec, withtycs = withtycs : Ast.tb list} =>
              AbstypeDec
              {abstycs = List.map (convertDb conversionInfo) abstycs, body = convertDec conversionInfo dec, withtycs = List.map (convertTb conversionInfo) withtycs}
          | Ast.DataReplDec (sym, path) => DataReplDec (sym, path)
          | Ast.DatatypeDec {datatycs = datatycs : Ast.db list, withtycs = withtycs : Ast.tb list} =>
              DatatypeDec
              {datatycs = List.map (convertDb conversionInfo) datatycs, withtycs = List.map (convertTb conversionInfo) withtycs}
          ...
          | Ast.FixDec {fixity = fixity : fixity, ops = ops : symbol list} =>
              FixDec {fixity = fixity, ops = ops}
          | Ast.FsigDec fsigbs => FsigDec (List.map (convertFsigb conversionInfo) fsigbs)
          | Ast.FunDec (fbs, tyvars) =>
              FunDec
              (List.map (convertFb conversionInfo) fbs, List.map (convertTyvar conversionInfo) tyvars)
          | Ast.LocalDec (d1, d2) =>
          ...
          | Ast.ValDec (vbs, tyvars) =>
              ValDec
              (List.map (convertVb conversionInfo) vbs, List.map (convertTyvar conversionInfo) tyvars)
          | Ast.ValrecDec (rvbs, tyvars) =>
              ValrecDec
              (List.map (convertRvb conversionInfo) rvbs, List.map (convertTyvar conversionInfo) tyvars)
          | Ast.MarkDec (dec, region) =>
              ...
              in
                case attachedComments of
                  ...
                  | comments =>
                      CommentDec
                      (String.concat (List.map (fn (_, x) => String.concat x) comments), MarkDec (convertedDec, region))
              end

    and convertVb conversionInfo vb =
        case vb of
            Ast.MarkVb (vb, region) => MarkVb (convertVb conversionInfo vb, region)
          | Ast.Vb {exp = exp : Ast.exp, lazyp = lazyp : bool, pat = pat : Ast.pat} =>
              Vb
              {exp = convertExp conversionInfo exp, lazyp = lazyp, pat = convertPat conversionInfo pat}

    and convertRvb conversionInfo rvb =
        case rvb of
            Ast.MarkRvb (rvb, region) => MarkRvb (convertRvb conversionInfo rvb, region)
          | Ast.Rvb {exp = exp : Ast.exp, fixity = fixity : (symbol * region) option, lazyp = lazyp : bool, resultty = resultty : Ast.ty option, var = var : symbol} =>
              Rvb
              {exp = convertExp conversionInfo exp, fixity = fixity, lazyp = lazyp, resultty = Option.map (convertTy conversionInfo) resultty, var = var}
    ...
    and convertFb conversionInfo (Ast.Fb (clauses, b)) =
      ...
      | convertFb conversionInfo (Ast.MarkFb (fb, region)) =
        MarkFb (convertFb conversionInfo fb, region)

    and convertClause conversionInfo (Ast.Clause {exp = exp : Ast.exp, pats = pats : Ast.pat Ast.fixitem list, resultty = resultty : Ast.ty option}) =
        Clause
        {exp = convertExp conversionInfo exp, pats = List.map (mapFixitem (convertPat conversionInfo)) pats, resultty = Option.map (convertTy conversionInfo) resultty}

    and convertTb conversionInfo (Ast.MarkTb (tb, region)) =
        MarkTb (convertTb conversionInfo tb, region)
      | convertTb conversionInfo (Ast.Tb {def = def : Ast.ty, tyc = tyc : symbol, tyvars = tyvars : Ast.tyvar list}) =
        Tb
        {def = convertTy conversionInfo def, tyc = tyc, tyvars = List.map (convertTyvar conversionInfo) tyvars}

    and convertDb conversionInfo (Ast.Db {lazyp = lazyp : bool, rhs = rhs : (symbol * Ast.ty option) list, tyc = tyc : symbol, tyvars = tyvars : Ast.tyvar list}) =
        Db
        {lazyp = lazyp, rhs = List.map (fn (sym, ty) => (sym, Option.map (convertTy conversionInfo) ty)) rhs, tyc = tyc, tyvars = List.map (convertTyvar conversionInfo) tyvars}
      | convertDb conversionInfo (Ast.MarkDb (db, region)) =
        MarkDb (convertDb conversionInfo db, region)

    and convertEb conversionInfo eb =
        case eb of
            Ast.EbDef {edef = edef : path, exn = exn : symbol} => EbDef {edef = edef, exn = exn}
          | Ast.EbGen {etype = etype : Ast.ty option, exn = exn : symbol} =>
    ...
    and convertStrb conversionInfo (Ast.MarkStrb (strb, region)) =
        MarkStrb (convertStrb conversionInfo strb, region)
      | convertStrb conversionInfo (Ast.Strb {constraint = constraint : Ast.sigexp Ast.sigConst, def = def : Ast.strexp, name = name : symbol}) =
        Strb
        {constraint = mapSigconst (convertSigexp conversionInfo) constraint, def = convertStrexp conversionInfo def, name = name}

    and convertFctb conversionInfo (Ast.Fctb {def = def : Ast.fctexp, name = name : symbol}) =
        Fctb {def = convertFctexp conversionInfo def, name = name}
      | convertFctb conversionInfo (Ast.MarkFctb (fctb, region)) =
        MarkFctb (convertFctb conversionInfo fctb, region)

    and convertSigb conversionInfo (Ast.MarkSigb (sigb, region)) =
        MarkSigb (convertSigb conversionInfo sigb, region)
      | convertSigb conversionInfo (Ast.Sigb {def = def : Ast.sigexp, name = name : symbol}) =
        Sigb {def = convertSigexp conversionInfo def, name = name}

    and convertFsigb conversionInfo (Ast.Fsigb {def = def : Ast.fsigexp, name = name : symbol}) =
        Fsigb {def = convertFsigexp conversionInfo def, name = name}
      | convertFsigb conversionInfo (Ast.MarkFsigb (fsigb, region)) =
        MarkFsigb (convertFsigb conversionInfo fsigb, region)

    and convertTyvar conversionInfo (Ast.MarkTyv (tyvar, region)) =
        MarkTyv (convertTyvar conversionInfo tyvar, region)
    ...
    and convertTy conversionInfo ty =
        case ty of
          ...
          | Ast.RecordTy tys =>
              RecordTy (List.map (fn (sym, ty) => (sym, convertTy conversionInfo ty)) tys)
